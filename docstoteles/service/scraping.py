...
class ScrapingService:
    def __init__(self):
        self.api_key = os.getenv("FIRECRAWL_API_KEY")  # para usar via API do firecrawl
        # self.api_url = os.getenv("FIRECRAWL_API_URL")  # para usar localmente , baixando do github do firecrawl

        self.app = FirecrawlApp(api_key=self.api_key)  # api_url=self.api_url, se for rodar localmente
    
    def scrape_website(self, url, collection_name):
        """Scraping completo em uma função"""
        try:
            # 1. Mapear URLs - CORREÇÃO AQUI
            map_result = self.app.map_url(url)  # pega os links e sublinks da URL
            ...
            os.makedirs(collection_path, exist_ok=True)  # criando a pasta caso nao exista
